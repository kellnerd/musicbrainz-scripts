...
```js
javascript:(function(){function a(o,e){return e.forEach(([e,t])=>{o=o.replace(e,t)}),o}const u={release:{label:{'\xa9':708,'\u2117':711,'licensed from':712,'licensed to':833,'distributed by':361,'marketed by':848}}};var e=/([^.,]+(?:, (?:LLP|Inc\.?))?)/;const s=new RegExp(/(\u2117\s*[&+]\s*\xa9|[\xa9\u2117])\s*(\d+)?\s+/.source+e.source,'g'),i=new RegExp(/(licen[sc]ed? (?:to|from)|(?:distributed|marketed) by)\s+/.source+e.source,'gi');function c(e){return a(e.toLowerCase().trim(),[[/licen[sc]ed?/g,'licensed']])}var l=prompt('Copyright notice:');l&&async function(e,t=!1){for(const s of e){var o='label',n=u.release[o],r=MB.entity(t?(await async function(e){const t=await fetch("/ws/js/label?q="+encodeURIComponent(e));return t.json()}(s.name))[0]:{name:s.name,entityType:o});for(const i of s.types){const c=(a=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:a,source:a.source,target:r})),l=c.relationship();l.linkTypeID(n[i]),l.entity0_credit(s.name),s.year&&(l.begin_date.year(s.year),l.end_date.year(s.year)),t?c.accept():((a=c).open(void 0),a.autocomplete.$input.focus(),a.autocomplete.search(),await function(t){return new Promise(e=>{t?t.$dialog.on('dialogclose',()=>{e()}):e()})}(c))}}var a}(function(e){const t=[];for(const n of(e=a(l,[[/\(C\)/gi,'\xa9'],[/\(P\)/gi,'\u2117'],[/\xab(.+?)\xbb/g,'$1']])).matchAll(s)){var o=n[1].split(/[&+]/).map(c);t.push({name:n[3],types:o,year:n[2]})}for(const r of e.matchAll(i))t.push({name:r[2],types:[c(r[1])]});return t}(l))})();
```
