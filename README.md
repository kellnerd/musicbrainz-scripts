...
```js
javascript:(function(){function c(t,e){return e.forEach(([e,r])=>{t=t.replace(e,r)}),t}var e=/(.+?(?:,?\s(?:LLC|LLP|(?:Inc|Ltd)\.?))?)(?:(?<=\.)|$|(?=,|\.|\sunder\s))/;const d=new RegExp(/([\xa9\u2117](?:\s*[&+]?\s*[\xa9\u2117])?)(?:.+?;)?\s*(\d{4}(?:\s*[,&]\s*\d{4})*)?(?:[^,.]*\sby)?\s+/.source+String.raw`(${e.source}(?:\s*/\s*${e.source})*)`,'gm'),l=new RegExp(/((?:(?:licen[sc]ed?\s(?:to|from)|(?:distributed|marketed)(?:\sby)?)(?:\sand)?\s)+)/.source+e.source,'gim');function u(e){return c(e.toLowerCase().trim(),[[/licen[sc]ed?/g,'licensed'],[/(distributed|marketed)(\sby)?/,'$1 by']])}const p={release:{artist:{'\xa9':709,'\u2117':710},label:{'\xa9':708,'\u2117':711,'licensed from':712,'licensed to':833,'distributed by':361,'marketed by':848}},recording:{artist:{'\u2117':869},label:{'\u2117':867}}};e=prompt('Copyright notice:');e&&async function(e){for(const i of e){var r='label',t=p.release[r],s=MB.entity({name:i.name,entityType:r});for(const a of i.types){const n=(o=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:o,source:o.source,target:s})),c=n.relationship();c.linkTypeID(t[a]),c.entity0_credit(i.name),i.year&&!Array.isArray(i.year)&&(c.begin_date.year(i.year),c.end_date.year(i.year)),(o=n).open(void 0),o.autocomplete.$input.focus(),o.autocomplete.search(),await function(r){return new Promise(e=>{r?r.$dialog.on('dialogclose',()=>{e()}):e()})}(n)}}var o}(function(e){const r=[];for(const s of(e=c(e,[[/\(C\)/gi,'\xa9'],[/\(P\)/gi,'\u2117'],[/\xab(.+?)\xbb/g,'$1'],[/for (.+?) and (.+?) for the world outside \1/g,'/ $2'],[/\u2117\s*(under\s)/gi,'$1'],[/(?<=\u2117\s*)digital remaster/gi,'']])).matchAll(d)){const o=s[3].split(/\/(?=\s|\w{2})/).map(e=>e.trim()),i=s[1].split(/[&+]|(?<=[\xa9\u2117])(?=[\xa9\u2117])/).map(u),a=s[2]?.split(/[,&]/).map(e=>e.trim());o.forEach(e=>{r.push({name:e,types:i,year:(e=a,Array.isArray(e)&&1===e.length?e[0]:e)})})}for(const n of e.matchAll(l)){var t=n[1].split(/\sand\s/).map(u);r.push({name:n[2],types:t})}return r}(e))})();
```
