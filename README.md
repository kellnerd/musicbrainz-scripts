...
```js
javascript:(function(){function a(o,e){return e.forEach(([e,t])=>{o=o.replace(e,t)}),o}var e=/(.+?(?:,?\s(?:LLC|LLP|(?:Inc|Ltd)\.?))?)(?:(?<=\.)|$|(?=,|\.|\sunder\s))/;const c=new RegExp(/([\xa9\u2117](?:\s*[&+]?\s*[\xa9\u2117])?)(?:.+?;)?\s*(\d{4})?(?:[^,.]*\sby)?\s+/.source+String.raw`(${e.source}(?:\s*/\s*${e.source})*)`,'gm'),d=new RegExp(/((?:(?:licen[sc]ed?\s(?:to|from)|(?:distributed|marketed)(?:\sby)?)(?:\sand)?\s)+)/.source+e.source,'gim');function l(e){return a(e.toLowerCase().trim(),[[/licen[sc]ed?/g,'licensed'],[/(distributed|marketed)(\sby)?/,'$1 by']])}const u={release:{label:{'\xa9':708,'\u2117':711,'licensed from':712,'licensed to':833,'distributed by':361,'marketed by':848}},recording:{label:{'\u2117':867}}};var p=prompt('Copyright notice:');p&&async function(e){for(const n of e){var t='label',o=u.release[t],r=MB.entity({name:n.name,entityType:t});for(const i of n.types){const a=(s=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:s,source:s.source,target:r})),c=a.relationship();c.linkTypeID(o[i]),c.entity0_credit(n.name),n.year&&(c.begin_date.year(n.year),c.end_date.year(n.year)),(s=a).open(void 0),s.autocomplete.$input.focus(),s.autocomplete.search(),await function(t){return new Promise(e=>{t?t.$dialog.on('dialogclose',()=>{e()}):e()})}(a)}}var s}(function(e){const t=[];for(const r of(e=a(p,[[/\(C\)/gi,'\xa9'],[/\(P\)/gi,'\u2117'],[/\xab(.+?)\xbb/g,'$1'],[/for (.+?) and (.+?) for the world outside \1/g,'/ $2'],[/\u2117\s*(under\s)/gi,'$1']])).matchAll(c)){const s=r[3].split(/\/(?=\s|\w{2})/g).map(e=>e.trim()),n=r[1].split(/[&+]|(?<=[\xa9\u2117])(?=[\xa9\u2117])/).map(l);s.forEach(e=>{t.push({name:e,types:n,year:r[2]})})}for(const i of e.matchAll(d)){var o=i[1].split(/\sand\s/).map(l);t.push({name:i[2],types:o})}return t}(p))})();
```
