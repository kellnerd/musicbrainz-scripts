...
```js
javascript:(function(){function i(o,e){return e.forEach(([e,t])=>{o=o.replace(e,t)}),o}var e=/(.+?(?:, (?:LLP|Inc\.?))?)(?=,|\.| under |$)/;const s=new RegExp(/(\u2117\s*[&+]\s*\xa9|[\xa9\u2117])\s*(\d+)?\s+/.source+e.source,'g'),a=new RegExp(/(licen[sc]ed? (?:to|from)|(?:distributed|marketed) by)\s+/.source+e.source,'gi');function c(e){return i(e.toLowerCase().trim(),[[/licen[sc]ed?/g,'licensed']])}const l={release:{label:{'\xa9':708,'\u2117':711,'licensed from':712,'licensed to':833,'distributed by':361,'marketed by':848}}};var d=prompt('Copyright notice:');d&&async function(e){for(const i of e){var t='label',o=l.release[t],r=MB.entity({name:i.name,entityType:t});for(const s of i.types){const a=(n=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:n,source:n.source,target:r})),c=a.relationship();c.linkTypeID(o[s]),c.entity0_credit(i.name),i.year&&(c.begin_date.year(i.year),c.end_date.year(i.year)),(n=a).open(void 0),n.autocomplete.$input.focus(),n.autocomplete.search(),await function(t){return new Promise(e=>{t?t.$dialog.on('dialogclose',()=>{e()}):e()})}(a)}}var n}(function(e){const t=[];for(const r of(e=i(d,[[/\(C\)/gi,'\xa9'],[/\(P\)/gi,'\u2117'],[/\xab(.+?)\xbb/g,'$1']])).matchAll(s)){var o=r[1].split(/[&+]/).map(c);t.push({name:r[3].trim(),types:o,year:r[2]})}for(const n of e.matchAll(a))t.push({name:n[2],types:[c(n[1])]});return t}(d))})();
```
