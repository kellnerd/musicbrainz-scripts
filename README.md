...
[![Install](https://img.shields.io/badge/Install-success.svg?style=for-the-badge&logo=tampermonkey)](dist/parseCopyrightText.user.js?raw=1)
[![Source](https://img.shields.io/badge/Source-grey.svg?style=for-the-badge&logo=github)](dist/parseCopyrightText.user.js)

```js
javascript:(function(){function r(o,e){return e.forEach(([e,t])=>{o=o.replace(e,t)}),o}const p={release:{label:{'\xa9':708,'\u2117':711,'licensed from':712,'licensed to':833,'marketed by':848}}};function i(e){return r(e.toLowerCase().trim(),[['licence','license']])}var s=prompt('Copyright text:');s&&async function(e,t=!1){for(const i of e){var o='label',n=p.release[o],a=MB.entity(t?(await async function(e){const t=await fetch(`/ws/js/label?q=${encodeURIComponent(e)}`);return t.json()}(i.name))[0]:{name:i.name,entityType:o});for(const s of i.types){const c=(r=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:r,source:r.source,target:a})),l=c.relationship();l.linkTypeID(n[s]),l.entity0_credit(i.name),i.year&&(l.begin_date.year(i.year),l.end_date.year(i.year)),t?c.accept():((r=c).open(void 0),r.autocomplete.$input.focus(),r.autocomplete.search(),await function(t){return new Promise(e=>{t?t.$dialog.on('dialogclose',()=>{e()}):e()})}(c))}}var r}(function(e){const t=[];for(const n of(e=r(s,[[/\(C\)/gi,'\xa9'],[/\(P\)/gi,'\u2117'],[/\xab(.+?)\xbb/g,'$1']])).matchAll(/([\xa9\u2117]|\u2117\s*[&+]\s*\xa9)\s*(\d+)\s+([^.,]+)/g)){var o=n[1].split(/[&+]/).map(i);t.push({name:n[3],types:o,year:n[2]})}for(const a of e.matchAll(/(licen[sc]ed (?:to|from)|marketed by)\s+([^.,]+)/gi))t.push({name:a[2],types:[i(a[1])]});return t}(s))})();
```
...
```js
javascript:(function(){const t=prompt('MBIDs of entities which should be related to this entity:');t&&async function(t,i,o=!1){for(var e of t){var n=await async function(){const t=await fetch(`/ws/js/entity/${e}`);return MB.entity(await t.json())}();const a=(e=void 0,e=MB.sourceRelationshipEditor??MB.releaseRelationshipEditor,new MB.relationshipEditor.UI.AddDialog({viewModel:e,source:e.source,target:n}));if(i){const r=a.relationship();r.linkTypeID(i),o&&a.changeDirection()}a.accept()}}(Array.from(t.matchAll(/[0-9a-f-]{36}/gm),t=>t[0]))})();
```
